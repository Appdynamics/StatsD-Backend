...
AppDynamics.prototype.post = function(data) {
	...
	if (debugConfig.postAgent) {
		...
		req.on('error', (error) => { this.errors++; console.error(`post error[${error}]`); })
		req.write(dataStr)
		req.end();
	}
	this.bytes += dataStr.length;
}; // post
...
AppDynamics.prototype.flush = function(timestamp, metrics) {
	...
	this.post(data);
	console.log(`AppDynamics post: ${this.count} ${this.errors} ${this.bytes} `, new Date(timestamp * 1000).toString());
}; // flush
